...
Rust workspace, 18 crates, version 2.1.0, MSRV 1.91. `emem-guard` is the verdict server for AI inference checkpoints (Anthropic Inference hooks, Claude Code hooks); it is a separate binary and shares no code path with the responder. The bulk of the code
...
| MCP tool registry (89 tools) | `crates/emem-mcp/src/lib.rs` |
