...
- **[Registries](./registries.md):** bands (43 cube slots, 124 wired names), algorithms (161), functions, sources (46), topics (27), schema, lcv-1, alphabet
...
- `POST /mcp` `tools/list` returns the 14 core tools by default, so a host's context carries about 39 KB of descriptors instead of 204 KB; `POST /mcp/full` advertises all 92 (14 core, 78 extended), and `{"tier":"core"|"extended"|"all"}` overrides either default. `tools/call` dispatches all 92 by name at both endpoints, so narrowing discovery removes no capability. `emem_tools` maps the surface: every tool declares one `dev.emem/shape` (the form of the answer: `scalar`, `raster`, `identity`, `token`, `proof`, and so on) and any number of overlapping `dev.emem/bundles` (the job: `tokenisation`, `verification`, `agent_to_agent`, `robotics`, …) in MCP-standard `_meta`, and both are filters
