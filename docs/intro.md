...
- *Content-addressed everywhere*. A `fact_cid` is 26 base32 chars; the same bytes hash to the same CID on every responder. `emem:fact:<cell64>:<fact_cid>` is the cite handle for one fact at one place; `emem:bundle:<bundle_cid>` for N facts at N places; `emem:entity:<entity_cid>` for a whole object (minted by `emem_entity`), the object you cite rather than only a fact. The pre-rename prefixes `memt:`, `memb:`, and `meme:` still resolve.
