...
- `POST /mcp` `tools/list` returns the 18 core tools by default, so a host's context carries about 75 KB of descriptors instead of 324 KB; `POST /mcp/full` advertises all 114 (18 core, 96 extended), and `{"tier":"core"|"extended"|"all"}` overrides either default. `tools/call` dispatches all 114 by name at both endpoints, so narrowing discovery removes no capability. `emem_tools` maps the surface: every tool declares one `dev.emem/shape` (the form of the answer: `scalar`, `raster`, `identity`, `token`, `proof`, and so on) and any number of overlapping `dev.emem/bundles` (the job: `tokenisation`, `verification`, `agent_to_agent`, `robotics`, …) in MCP-standard `_meta`, and both are filters
