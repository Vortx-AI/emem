...
One sentence first: emem is a shared memory for AI agents that connects facts and gets better over time — and every answer is signed, so anyone can check it without trusting the server.
...
- `POST /mcp` `tools/list` returns all 80 MCP tools (10 core, 70 extended) by default; `{"tier":"core"}` narrows to the 10 essentials
- `POST /mcp` `resources/list` — 7 anchor resources + 3 URI templates (`memory://emem/cell/<cell64>`, `memory://emem/fact/<cid>`, `memory://emem/bundle/<token>`)
