...
port 5051. The live surface documents 160 paths under
`/v1/*` (169 total in `/openapi.json`), 108 MCP tools (16 core, 92 extended, with
...
Version 2.3.0, MSRV Rust 1.91. No API keys. Agent-memory writes (the memory_* file verbs) ship over MCP; fact attestation stays REST-only (`POST /v1/attest`) because it needs an Ed25519 secret no LLM host can manage safely.
...
| Resource | Live count |
|---|---|
| REST paths (OpenAPI) | 169 documented, 160 under `/v1/*` |
