...
- Canonical catalog: `pub const TOOLS: &[ToolDescriptor]` in
  ...
  hint flags, tier). 114 tools: 18 core, 96 extended. Helpers: `lookup`
...
- Two endpoints, one dispatcher: `/mcp` (`mcp_jsonrpc`) advertises the core
  ...
  advertises all 114. Both funnel into `mcp_jsonrpc_inner` (:14891) with a
...
`scripts/sync_counts.py --check` verifies the canonical counts (114 tools,
172 /v1 paths, 46 sources, 43 slots, 129 wired bands, 168 algorithms, 27
