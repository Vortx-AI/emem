...
- Canonical catalog: `pub const TOOLS: &[ToolDescriptor]` in
  ...
  hint flags, tier). 115 tools: 16 core, 97 extended. Helpers: `lookup`
...
- Two endpoints, one dispatcher: `/mcp` (`mcp_jsonrpc`) advertises the core
  ...
  advertises all 115. Both funnel into `mcp_jsonrpc_inner` (:14891) with a
...
`scripts/sync_counts.py --check` verifies the canonical counts (115 tools,
173 /v1 paths, 46 sources, 43 slots, 129 wired bands, 168 algorithms, 27
