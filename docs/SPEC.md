...
The hosted responder sets no cookies of its own. The HTML landing page at `/` (and **only** that page; not `/v1/*`, not `/mcp`, not `/openapi.json`, not the markdown surfaces, not `.well-known/*`) loads Google Analytics 4 with the operator's configured measurement ID (set via env var `EMEM_GA_MEASUREMENT_ID`; the live ID is published machine-readably at `/.well-known/agent-card.json` under `provider.data_protection.third_party_analytics[].measurement_id`) under Consent Mode v2 with default-denied for every advertising and analytics storage purpose. A consent banner is rendered for human visitors with JS-capable browsers; AI crawlers do not run JS and therefore do not see the banner. The banner offers two equally-prominent buttons:

- **Reject** keeps every storage purpose denied. GA cookies are not set; no IP is transmitted; only cookieless aggregated pings continue. The decision is recorded in a first-party cookie `emem_consent=reject` (Path=/, Max-Age=180 days, SameSite=Lax, Secure).
- **Accept** flips `analytics_storage` and `functionality_storage` to `granted`. From that moment GA4 sets `_ga` (2-year) and `_ga_<container>` (2-year) cookies, transmits the IP-anonymised payload, and emits regular analytics events. The decision is recorded as `emem_consent=accept`.

The banner is not a cookie wall: every endpoint and link works regardless of consent state. The Esc key dismisses with Reject (default-deny on accidental dismiss). A footer **Manage cookies** link deletes `emem_consent` (`Max-Age=0`) and reopens the banner so the user can change their decision at any time.

The `emem_consent` cookie is exempt from prior consent under ePrivacy Art. 5(3) "strictly necessary for the service requested" because remembering a consent decision is required to honour it. The decision moved from `localStorage` to a first-party cookie in 2026-05 because EU-strict browser configurations (Firefox Strict mode, Brave Shields, the "delete site data on close" defaults common in the EEA) were clearing `localStorage` between sessions, causing the banner to re-prompt on every refresh. Cookies survive those configurations reliably while remaining strictly necessary. No `localStorage`, `sessionStorage`, or `IndexedDB` entries, no fingerprinting probes. Static assets (`/favicon.svg`, `/og-image.svg`) are served from the same origin.

The measurement ID itself is configured per operator: forks of the reference build do not inherit the canonical responder's GA stream because the repo holds only a placeholder (`__EMEM_GA_ID__`). When `EMEM_GA_MEASUREMENT_ID` is unset, the entire GA `<script>` block is stripped from the served HTML at startup; no GA traffic is generated.

`curl -I https://emem.dev/` returns no `Set-Cookie` header (the consent cookie is set client-side by JavaScript after the banner click; the server does not emit it). Full disclosure at `/privacy`; machine-readable in `/.well-known/agent-card.json` under `provider.data_protection.third_party_analytics[].consent_banner`.
