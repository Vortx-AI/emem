...
|---|---|---|---|
| P1-1 | No `outputSchema` on any of 105 tools, no `structuredContent` in responses | `tools/list` and inspect any descriptor | **PARTLY NOT REPRODUCED, rest BLOCKED**: `structuredContent` IS emitted, on every result whose two-copy envelope fits the wire budget. It is dropped only when it would not, because the host truncates an oversized payload silently and mid-token. `outputSchema` stays undeclared for that reason: the spec requires conforming `structuredContent` whenever `outputSchema` is present, so declaring it would make the large tools non-compliant, and forcing the mirror universal would mean slimming answers to carry a redundant copy. Return shape is published per tool as `_meta["dev.emem/shape"]` |
