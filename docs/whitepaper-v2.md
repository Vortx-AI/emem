...
responder refutes it from its own annotations: **24 of 114 tools carry
...
loop, about 40 KB, and `POST /mcp/full` advertises all 114.

Narrowing discovery removes no capability: **`tools/call` dispatches all
114 by name at either endpoint**, and an explicit
...
| §15: "MCP tools are a strict read-only subset of REST; writes go through REST only" | **24 of 114 MCP tools write** (§11.1), including the five memory verbs, `emem_entity`, `emem_entity_link`, and `emem_derive`. |
