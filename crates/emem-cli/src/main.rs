...
#[derive(Parser, Debug)]
#[command(name = "emem", version, about = "emem agent-native spatial memory protocol")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Bands     => println!("{}", serde_json::to_string_pretty(&*emem_core::bands::DEFAULT)?),
        Cmd::Functions => println!("{}", serde_json::to_string_pretty(&*emem_core::functions::DEFAULT)?),
        Cmd::Sources   => println!("{}", serde_json::to_string_pretty(&*emem_core::sources::DEFAULT)?),
        Cmd::Errors    => {
            use emem_core::ErrorCode::*;
            let codes = [
                InvalidCell, InvalidResolution, TslotMismatch, BandNotInRegistry,
                FunctionNotInRegistry, SourceSchemeUnknown, CidNotFound,
                RegistryCidUnknown, SchemaCidUnknown,
                PrivacyRefused, LevelTooLow, AttesterRevoked, Unauthorized,
                ClaimUndecidable, BadSignature, BadMerkleProof,
                CanonicalEncodingDivergence,
                SourceFetchFailed, SourceFormatMismatch, ComputeTimeout,
                ComputeQuotaExceeded, RateLimited,
                CacheError, Internal,
            ];
        ...
        Cmd::Keygen => {
            ...
            let secret_b32 = data_encoding::BASE32_NOPAD.encode(&signing.to_bytes()).to_lowercase();
            let pub_b32 = data_encoding::BASE32_NOPAD.encode(signing.verifying_key().as_bytes()).to_lowercase();
            println!("{}", serde_json::to_string_pretty(&serde_json::json!({
