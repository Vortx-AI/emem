...
async fn main() -> anyhow::Result<()> {
    ...
    for (name, lat, lng, peak_m) in places {
        ...
        let range_strip = http
            .get(&url)
            .header("range", "bytes=1048576-1105919")
            .send()
            .await?;
        let strip_status = range_strip.status().as_u16();
        let strip_bytes = range_strip.bytes().await?;
        let range_status = if hdr_status == 206 && strip_status == 206 {
            206
        } else {
            hdr_status.max(strip_status)
        };
        ...
        println!(
            "    Range[0..{n}] status={range_status} blake3={}",
            &bytes_b3[..16]
        );
        ...
        let source = Source {
            ...
            cid: None,
            hash: Some(hash32),
            captured_at: if last_modified.is_empty() {
                None
            } else {
        ...
        real_facts.push(Fact::Primary(PrimaryFact {
            ...
            derivation: Derivation {
                fn_key: "copdem.head_and_range@1".into(),
                args: None,
            },
        ...
        real_facts.push(Fact::Primary(PrimaryFact {
            ...
            sources: vec![source.clone()],
            derivation: Derivation {
                fn_key: "byte_histogram_16@1".into(),
                args: None,
            },
            privacy_class: "public".into(),
            schema_cid: SchemaCid::new(&schema_cid),
    ...
    let att_resp = trace
        .post_cbor(&http, "02_attest_real_cbor", "/v1/attest_cbor", &att_cbor)
        .await?;
    ...
    trace
        .post_json(
            &http,
            "04_compare_fuji_vs_everest",
            "/v1/compare",
            json!({"a": &fuji_cell, "b": &everest_cell, "family": "copdem30m.byte_histogram_v1"}),
        )
        .await?;
    ...
    trace
        .post_json(
            &http,
            "05_compare_fuji_vs_grand_canyon",
            "/v1/compare",
            json!({"a": &fuji_cell, "b": &gc_cell, "family": "copdem30m.byte_histogram_v1"}),
        )
        .await?;
    ...
    trace
        .post_json(
            &http,
            "06_find_similar_to_everest",
            "/v1/find_similar",
            json!({"key": &everest_cell, "k": 3, "band": "copdem30m.byte_histogram_v1"}),
        )
        .await?;
    ...
    trace
        .post_json(
            ...
            "07_query_region_dem",
            "/v1/query_region",
            json!({"geometry": region, "bands": ["copdem30m.provenance"]}),
        )
        .await?;
    ...
    trace
        .post_json(
            &http,
            "08_verify_ifd_offset",
            "/v1/verify",
            json!({"cell": &fuji_cell,
               "claim": {"band":"copdem30m.byte_histogram_v1","op":"gt","value":-1.0,"tslot":0}}),
        )
        .await?;
    ...
    trace
        .post_json(
            &http,
            "09_intent_what_is_here",
            "/v1/intent",
            json!({"type":"what_is_here","cell": &gc_cell}),
        )
        .await?;
    ...
    trace
        .post_json(
            &http,
            "11_mcp_recall_everest",
            "/mcp",
            json!({"jsonrpc":"2.0","id":2,"method":"tools/call",
               "params":{"name":"emem.recall","arguments":{"cell": &everest_cell}}}),
        )
        .await?;
    ...
    trace.bands = vec![
        "copdem30m.provenance".into(),
        "copdem30m.byte_histogram_v1".into(),
    ];
    trace.tslots = vec![0];
    trace.write()?;
...
impl TraceIndex {
    ...
    fn write(&self) -> anyhow::Result<()> {
        ...
        for (n, c) in &self.places {
            md.push_str(&format!("| {n} | `{c}` |\n"));
        }
        ...
        for (_, st) in by_step.iter() {
            ...
            if let Some(rid) = &st.request_id {
                md.push_str(&format!("- request_id = `{rid}`\n"));
            }
