...
pub fn latlng_to_utm(lat_deg: f64, lon_deg: f64, force_zone: Option<u8>) -> UtmCoord {
    let zone = force_zone.unwrap_or_else(|| {
        let mut z = ((lon_deg + 180.0) / 6.0).floor() as i32 + 1;
        if z < 1 { z = 1; }
        if z > 60 { z = 60; }
        z as u8
    });
    let hemi = if lat_deg >= 0.0 { Hemi::North } else { Hemi::South };
    let epsg = match hemi { Hemi::North => 32600, Hemi::South => 32700 } + zone as u32;
    let (e, n) = forward_tm_wgs84(lat_deg, lon_deg, zone);
    UtmCoord { easting: e, northing: n, zone, hemi, epsg }
}
...
pub fn latlng_to_utm_with_epsg(lat_deg: f64, lon_deg: f64, epsg: u32) -> Option<UtmCoord> {
    ...
    let _ = hemi;
    Some(UtmCoord { easting: e, northing: n_pre, zone, hemi, epsg })
}
...
mod tests {
    ...
    fn cambridge_uk_zone31() {
        ...
        assert!(approx(u.northing, 5787777.0, 5.0), "northing={}", u.northing);
    }
    ...
    fn cambridge_uk_force_zone30() {
        ...
        assert!(approx(u.northing, 5788467.0, 5.0), "northing={}", u.northing);
    }
}
