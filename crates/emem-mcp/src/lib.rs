...
"head_is_witnessed":{"type":"boolean","description":"False whenever no witness has co-signed the current head. A witness attests only the prefix it signed. This counts the responder's OWN co-signatures too, and its write-liveness canary makes one every two minutes, so it is nearly always true and does not measure outside oversight. Use head_is_independently_witnessed."},
...
"head_is_independently_witnessed":{"type":"boolean","description":"The same question with self_witness_pubkey_b32 excluded. This is the one that bears on split view: a node cannot catch itself serving two histories."},
"freshest_independent_witness_entries_behind":{"type":["integer","null"],"description":"How far behind the freshest witness that is not this responder is. null when nobody else has ever co-signed."},
"independent_witness_count":{"type":"integer","description":"Distinct witness keys that are not this responder's."},
"independent_cosignature_count":{"type":"integer","description":"Co-signatures made by keys other than this responder's, over the whole store rather than the returned page."},
"self_witness_pubkey_b32":{"type":["string","null"],"description":"The responder's own declared witness key, published so the filter is reproducible. null when the operator declared none, in which case the independent_* fields counted every row."},
...
const SCHEMA_MEMORY_BUNDLE: &str = r#"{"type":"object","properties":{
"fact_cids":{"type":"array","minItems":1,"maxItems":256,"items":{"type":"string"},"description":"Instead of triples: the exact facts to bind, by cid. A triple is resolved through recall and can bind a newer fact than the one you showed; a cid binds that fact and no other. Pass triples or fact_cids, not both."},
...
pub const TOOLS: &[ToolDescriptor] = &[
    ...
    ToolDescriptor {
        ...
        description: "The map of emem's tool surface, and the only tool you need to find the rest: the working loop in the order you walk it (name, ground, cite, resolve, verify, check for drift), then every other tool grouped by the question it answers, each with its one-line trigger. Pass `name` for one tool's full schema and a runnable example. IF YOU ARE READING A LIST OF {TOOL_CORE} TOOLS, YOU ARE SEEING A CURATED SUBSET OF {TOOL_TOTAL}, NOT THE WHOLE SURFACE; hosts strip `_meta`, so the count is repeated here. Every catalogued tool stays callable by name through tools/call at either endpoint.",
        when_to_use: "Call FIRST when you do not know which tool answers the question, or need a capability absent from your list: absent from the list is not absent from the server. `q` searches by topic (`ndvi`, `flood`, `verify`), `name` returns one exact schema, no arguments returns the whole map. /mcp/full registers the full catalog; emem_ask answers in one shot without picking a primitive.",
    ...
    ToolDescriptor {
        ...
        output_schema: None,
        example_args: r#"{"cell":"defi.zb64a.cAzU.zfa27","max_cloud":20}"#,
        level: "L0", category: ToolCategory::Read,
    read_only_hint: true, destructive_hint: false, idempotent_hint: true, open_world_hint: true,
    tier: "extended",
    },
