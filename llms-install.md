...
- `https://emem.dev/mcp` advertises a 18-tool core loop, so a client
  ...
  megabyte.
- `https://emem.dev/mcp/full` advertises all 114.

The split is listing-only. Every one of the 114 tools is callable by name at
...
is identical: same endpoint, same 114 tools, no key to read.
