...
() => {
  ...
  for (const e of els) {
    ...
    boxes.push({
      ...
      // textContent points at the wrong one. Carry a real path.
      path: (() => {
        const seg = [];
...
def run_control(ctx):
    ...
    for name, html, want_failures in (
        ...
        n = len(failures(pg.evaluate(MEASURE)))
        got = n > 0
...
def main():
    ...
    # sweep.
    print(f"TOTAL {total} real" + (f"  ({errors} pages NOT MEASURED)" if errors else ""))
    return total + errors
