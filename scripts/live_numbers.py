...
def main():
    ...
    # Elements the page marks as awaiting a live value.
    pending = re.findall(r'<([a-z]+)([^>]*\bdata-pending\b[^>]*)>(.*?)</\1>', s, re.S)
    if not pending:
    ...
    # Anything filled by the live-numbers script.
    ...
    print(f"live numbers: {len(pending_ids)} figures declared live, "
